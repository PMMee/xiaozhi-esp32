...
static uint16_t bq27220_read_u16(bq27220_handle_t bq_handle, uint8_t address)
{
    ...
    esp_err_t res = bq_i2c_read(bq_data, address, (uint8_t *)&buf, 2);
    PRINT_ERROR(res);
    return buf;
}
...
static esp_err_t bq27220_control(bq27220_handle_t bq_handle, uint16_t control)
{
    ...
    esp_err_t res = bq_i2c_write(bq_data, COMMAND_CONTROL, data, 2);
    PRINT_ERROR(res);
    return res;
}
...
esp_err_t bq27220_set_parameter_u16(bq27220_handle_t bq_handle, uint16_t address, uint16_t value)
{
    ...
    buffer[3] = value & 0xFF;
    res = bq_i2c_write(bq_data, COMMAND_SELECT_SUBCLASS, buffer, 4);
    PRINT_ERROR(res);
    delay_ms(10);
    ...
    buffer[0] = checksum;
    buffer[1] = 6;
    res = bq_i2c_write(bq_data, COMMAND_MAC_DATA_SUM, buffer, 2);
    PRINT_ERROR(res);
    delay_ms(10);
    return res;
}
...
uint16_t bq27220_get_parameter_u16(bq27220_handle_t bq_handle, uint16_t address)
{
    ...
    buffer[0] = address & 0xFF;
    buffer[1] = (address >> 8) & 0xFF;
    res = bq_i2c_write(bq_data, COMMAND_SELECT_SUBCLASS, buffer, 2);
    PRINT_ERROR(res);
    delay_ms(10);

    res = bq_i2c_read(bq_data, COMMAND_MAC_DATA, buffer, 2);
    PRINT_ERROR(res);
