...
| 类型 | 内容 | 涉及文件 |
|------|------|---------|
| 🆕 | **音乐暂停/续播功能** — 唤醒词打断后说"继续播放"能从断点恢复 | `esp32_music.h/cc`, `application.cc`, `music.h`, `dual-screen.cc` |
| 🆕 | 音乐结束原因区分 — `Normal`(自然播完) / `Stopped`(用户中断) / `Error`(网络中断) → 三种不同 AI 交互 | `esp32_music.h/cc` |
| 🐛 | 唤醒词打断音乐后说"继续播放"恢复播放（BUG 修复） | `esp32_music.h/cc` |
| 🐛 | 续播解码器死循环 — `need_more` 标志强制读 HTTP（续播从文件中间开始时 underflow 缓冲区未满不退化为死循环） | `esp32_music.cc` |
| 🐛 | 续播 Range 头在 302 重定向后被清零 — 改为所有重定向完成后才清零 | `esp32_music.cc` |
| 🐛 | `Start()` 未清 `current_music_url_` → 新歌总是放旧歌 — 已修复 | `esp32_music.cc` |
| 🐛 | `Start()` 未重置 `end_reason_` → 自然播完后不触发 AI — 已修复 | `esp32_music.cc` |
| 🐛 | `IsPlaying()` 不含暂停态 → AI 说"没有在播放" — 已修复 | `esp32_music.cc` |
| 🐛 | 歌手搜索失败 — API 返回"歌名不见了？qwq" → 无歌名时自动用歌手名作 `&name=` 参数 | `esp32_music.cc` |
| 🐛 | BQ27220 I2C 无限超时导致未焊芯片的板子卡死启动 — probe/读写改为 100ms/500ms 超时 | `bq27220.c` |
| 🐛 | BQ27220 `WAIT_OPERATION_DONE` 宏超时 `ret=false` 被下一行无条件覆盖 | `bq27220.c` |
| 🔧 | 音乐播放不杀 MQTT — `SuspendAudioChannel`/`ResumeAudioChannel` 替代 `ResetProtocol`/`Reconnect` | `application.h/cc`, `dual-screen.cc` |
| 🔧 | WiFi 热点名 `Xiaozhi-xxxx` → `Aethertoys-xxxx` | `wifi_board.cc` |
| 🔧 | 音乐播放结束自动恢复对话（三种场景差异化消息） | `esp32_music.cc`, `application.cc` |

### 2026-07-01

| 类型 | 内容 |
|------|------|
| 🆕 | **闹钟系统** — `AlarmManager` 完整 CRUD + MCP 工具 + NVS 持久化 |
| 🆕 | 唤醒词打断闹钟 + BOOT 按钮停止闹钟 |
| 🐛 | 唤醒词打断闹钟后设备不响应 — `CloseAudioChannel()` 清除脏状态 |
| 🆕 | 音乐源切换 `MUSIC_SOURCE_TYPE` (kw/qq/wy/kg) |
| 🐛 | QQ 音乐 CDN 限速 → 默认切酷我 |
| 🐛 | `%lld` ESP-IDF 格式符 → `%d` + `(int)` 强转 |
...
| 电量检测 | ADC_CHANNEL_2 (IO3) / BQ27220 I2C | 支持充电完成检测 (IO13)，可选 BQ27220 库仑计 |
| 灯光 | WS2812B RGB | 单线控制, GPIO 15 |
| 触摸 | TOUCH 4/5 | 铜箔电容触摸 |
| 网络 | WiFi 2.4GHz | 纯WiFi, 无4G模块 |

**配网热点名**: `Aethertoys-XXXX`（XXXX 为 MAC 末四位），配置在 `wifi_board.cc`。

**关键设计决策**:
- 采样率统一为 **24kHz**（`AUDIO_INPUT_SAMPLE_RATE` / `AUDIO_OUTPUT_SAMPLE_RATE`）
- 显示使用 SPI 40MHz 模式（GC9D01 安全上限），独立左右屏片选
- 背光支持独立 PWM 控制，空闲自动调暗（20s 后降至 10%）
- 软件关机通过 GPIO 21 输出低电平触发
- BQ27220 电量计为**可选器件**，未焊接时启动自动跳过（100ms probe 超时后降级为 ADC 电量检测）
...
| MCP 工具名 | 功能 | 参数 |
|-----------|------|------|
| `self.music.play` | 搜索并播放音乐（断开音频通道后独占播放） | `song_name`, `artist_name` |
| `self.music.search` | 仅搜索不播放（异步超时保护） | `song_name`, `artist_name` |
| `self.music.stop` | 完全停止播放（清除暂停状态） | 无 |
| `self.music.resume` | 恢复已暂停的音乐（从断点续播） | 无 |

**`self.music.resume` 行为**：暂停过的音乐 → `Schedule` + `SuspendAudioChannel` → 断点续播。未暂停 → 返回 `{"ok":false}`。
...
**音乐播放结束后的网络清理**：已改用 `SuspendAudioChannel`/`ResumeAudioChannel` 复用 MQTT 连接，不再需要 2s 延迟和 TCP 清理。

### 4.6 暂停/续播功能

> **实现文件**: `main/boards/common/esp32_music.cc`

**原理**：暂停时保存 CDN 直链 URL + 已下载字节偏移 + 歌名/歌手；续播时用 HTTP `Range: bytes=<offset>-` 请求从断点继续下载。

```
正常播放:  Start("稻香", "周杰伦") → 搜索 → CDN直链 → 流式下载+解码
                                          │
唤醒词打断: Pause()                      │
            ├─ resume_url_ = CDN直链      │
            ├─ resume_offset_ = 200244    │  ← 已下载 200KB
            ├─ paused_ = true             │
            └─ Close HTTP                 │
                                          │
继续播放:   Resume()                      │
            ├─ 恢复 current_music_url_    │
            ├─ MusicTaskFunc 跳过搜索     │
            └─ HTTP GET + Range:bytes=200244- → CDN 返回 206 + 剩余数据
```

**状态流转**:

| 场景 | 入口 | `end_reason_` | `paused_` | 退出行为 |
|------|------|:---:|:---:|------|
| 点歌播放 | `Start()` | `Normal` | `false` | 播完 → `ResumeAudioChannel("音乐播放完毕")` |
| 唤醒词打断 | `Pause()` | `Stopped` | `true` | skip（唤醒词独占通道） |
| 继续播放 | `Resume()` | `Normal` | `false` | 同上 → 可多次续播 |
| 停止播放 | `Stop()` | `Stopped` | `false` | `CanResume()=false` |
| 网络中断 | 重试耗尽 | `Error` | `false` | `ResumeAudioChannel("音乐播放中断，网络不太好")` |

**关键 Bug 修复**:

| Bug | 根因 | 修复 |
|-----|------|------|
| 新歌总是放旧歌 | `Start()` 未清 `current_music_url_` → 任务以为这是续播 | `Start()` 清空全部 pause/url 状态 |
| 自然播完不触发 AI | `Start()` 未重置 `end_reason_` → 残留 `Stopped` | `Start()` 设 `end_reason_=Normal` |
| AI 说"没有在播放"但实际暂停中 | `IsPlaying()` 只看 `playing_` | `IsPlaying()` 包含 `paused_` |
| 续播解码器死循环 | underflow 时 `buf_pos=2353>2048` 不进 HTTP Read → 死解码 | `need_more` 标志强制下一轮读 HTTP |
| 续播从头开始放 | Range 头在第一次 302 后被清零 | 所有重定向完成后才清零 |

**歌手搜索修复**: shybot API 强制要求 `&name` 参数，只传 `&singer` 返回 "歌名不见了？qwq"。修复后无歌名时自动用歌手名作为 `&name` 参数。
...
#### Q9: 未焊 BQ27220 芯片的板子启动卡在 "Backlight: Set brightness to 75"

**现象**: 焊了 2 块新板子，一块正常启动，另一块卡死不动。

**日志特征**: 最后一行是 `Backlight: Set brightness to 75`，之后无输出。

**根因**: BQ27220 驱动中 I2C 读写（`bq_i2c_write`/`bq_i2c_read`）和 probe（`i2c_master_probe`）超时参数为 `-1`（`portMAX_DELAY` = 无限等待）。未焊芯片时 I2C 无 ACK 响应 → 永久阻塞。

**修复** (`main/boards/aethertoys-wifi-cn/bq27220.c`):
- `i2c_master_probe` 超时: `-1` → `pdMS_TO_TICKS(100)` (100ms)
- `bq_i2c_write` / `bq_i2c_read` 超时: `-1` → `pdMS_TO_TICKS(500)` (500ms)
- `WAIT_OPERATION_DONE` 宏: 超时 `ret=false` 被下一行 `ret=true` 无条件覆盖 → 改为 `if(timeout==0){ret=false;}else{ret=true;}`

修复后日志: `BQ27220: BQ27220 not found at 0x55 (not soldered?)` → 正常继续启动。

#### Q10: 搜索歌手失败，返回"歌名不见了？qwq"

**现象**: 说"放一首 NewJeans 的歌"，AI 调 `self.music.search` 失败。

**日志特征**: `[SEARCH] Raw body: [歌名不见了？qwq]` (21 bytes)

**根因**: shybot.top API 强制要求 `&name` 参数。只传 `&singer=NewJeans` 时 API 不执行搜索，直接返回错误文本。

**修复** (`esp32_music.cc`): 无 `song_name` 时自动用 `artist_name` 作为 `&name=<artist_name>`。

### 配网热点名

热点前缀配置在 `main/boards/common/wifi_board.cc`：
```cpp
config.ssid_prefix = "Aethertoys";  // 热点名格式: Aethertoys-XXXX
```
后缀为 MAC 地址末 4 位自动追加。
